defmodule Mix.Tasks.PinStripe.SyncWebhookHandlersTest do
  use ExUnit.Case

  describe "Docs module" do
    test "has short_doc/0" do
      assert is_binary(Mix.Tasks.PinStripe.SyncWebhookHandlers.Docs.short_doc())
    end

    test "has example/0" do
      assert is_binary(Mix.Tasks.PinStripe.SyncWebhookHandlers.Docs.example())
    end

    test "has long_doc/0" do
      assert is_binary(Mix.Tasks.PinStripe.SyncWebhookHandlers.Docs.long_doc())
    end
  end
  ...
  if Code.ensure_loaded?(Igniter) do
    describe "info/2" do
      test "returns valid Task.Info struct" do
        info = Mix.Tasks.PinStripe.SyncWebhookHandlers.info([], nil)

        assert info.group == :pin_stripe
        assert "pin_stripe.gen.handler" in info.composes
